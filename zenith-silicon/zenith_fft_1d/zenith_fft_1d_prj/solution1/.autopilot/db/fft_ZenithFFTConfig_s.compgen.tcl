# This script segment is generated automatically by AutoPilot

# === IP Block fft<ZenithFFTConfig> ===
set id 40
set name fft_1d_top_fft_ZenithFFTConfig_s
set cppname "fft<ZenithFFTConfig>"
set corename Vivado_FFT
set op ipcall
set stage_num 0
set ipPortList { \
    {\
        name ap_clk \
        width 1 \
        signed 1 \
        dir IN \
        conntype clock \
        type STD_LOGIC \
        conn "" \
    }\
    {\
        name ap_rst \
        width 1 \
        signed 1 \
        dir IN \
        conntype reset \
        type STD_LOGIC \
        conn "" \
    }\
    {\
        name ap_ce \
        width 1 \
        signed 1 \
        dir IN \
        conntype ce \
        type STD_LOGIC \
        conn "" \
    }\
    {\
        name ap_start \
        width 1 \
        signed 1 \
        dir IN \
        conntype start \
        type STD_LOGIC \
        conn "" \
    }\
    {\
        name ap_done \
        width 1 \
        signed 1 \
        dir OUT \
        conntype port \
        type STD_LOGIC \
        conn "" \
    }\
    {\
        name ap_idle \
        width 1 \
        signed 1 \
        dir OUT \
        conntype port \
        type STD_LOGIC \
        conn "" \
    }\
    {\
        name ap_ready \
        width 1 \
        signed 1 \
        dir OUT \
        conntype port \
        type STD_LOGIC \
        conn "" \
    }\
    {\
        name fft_in_stream_dout \
        width 32 \
        signed 1 \
        dir IN \
        conntype port \
        type STD_LOGIC_VECTOR \
        conn "" \
    }\
    {\
        name fft_in_stream_empty_n \
        width 1 \
        signed 1 \
        dir IN \
        conntype port \
        type STD_LOGIC \
        conn "" \
    }\
    {\
        name fft_in_stream_read \
        width 1 \
        signed 1 \
        dir OUT \
        conntype port \
        type STD_LOGIC \
        conn "" \
    }\
    {\
        name fft_out_stream_din \
        width 32 \
        signed 1 \
        dir OUT \
        conntype port \
        type STD_LOGIC_VECTOR \
        conn "" \
    }\
    {\
        name fft_out_stream_full_n \
        width 1 \
        signed 1 \
        dir IN \
        conntype port \
        type STD_LOGIC \
        conn "" \
    }\
    {\
        name fft_out_stream_write \
        width 1 \
        signed 1 \
        dir OUT \
        conntype port \
        type STD_LOGIC \
        conn "" \
    }\
    {\
        name fft_config_stream_dout \
        width 16 \
        signed 1 \
        dir IN \
        conntype port \
        type STD_LOGIC_VECTOR \
        conn "" \
    }\
    {\
        name fft_config_stream_empty_n \
        width 1 \
        signed 1 \
        dir IN \
        conntype port \
        type STD_LOGIC \
        conn "" \
    }\
    {\
        name fft_config_stream_read \
        width 1 \
        signed 1 \
        dir OUT \
        conntype port \
        type STD_LOGIC \
        conn "" \
    }\
}

set ipParamList { \
    aclken {true} \
    aresetn {true} \
    butterfly_type {0} \
    channels {1} \
    complex_mult_type {1} \
    cyclic_prefix_insertion {0} \
    data_format {fixed_point} \
    implementation_options {2} \
    input_width {16} \
    memory_options_data {0} \
    memory_options_hybrid {0} \
    memory_options_phase_factors {0} \
    memory_options_reorder {0} \
    number_of_stages_using_block_ram_for_data_and_phase_factors {1} \
    output_ordering {1} \
    output_width {16} \
    ovflo {0} \
    phase_factor_width {16} \
    rounding_modes {1} \
    run_time_configurable_transform_length {0} \
    scaling_options {0} \
    super_sample_rates {1} \
    systolicfft_inv {0} \
    target_clock_frequency {150} \
    throttle_scheme {nonrealtime} \
    transform_length {1024} \
    xk_index {0} \
}

if {${::AESL::PGuard_simmodel_gen}} {
if {[info proc ::IP::FFT::gen] == "::IP::FFT::gen"} {
eval "::IP::FFT::gen { \
    view sim \
    id ${id} \
    name ${name} \
    reset_level 1 \
    sync_rst true \
    corename ${corename} \
    op ${op} \
    stage_num ${stage_num} \
    ip_param_list \{${ipParamList}\} \
    ip_port_list \{${ipPortList}\} \
}"
} else {
puts "@W \[IMPL-117\] Cannot find ::IP::FFT::gen, check your lib."
}
}


if {${::AESL::PGuard_rtl_comp_handler}} {
	::AP::rtl_comp_handler ${name} BINDTYPE op TYPE ip_block_$corename
}


if {${::AESL::PGuard_autocg_gen}} {
if {[info proc ::IP::FFT::gen] == "::IP::FFT::gen"} {
eval "::IP::FFT::gen { \
    view impl \
    id ${id} \
    name ${name} \
    reset_level 1 \
    sync_rst true \
    corename ${corename} \
    op ${op} \
    stage_num ${stage_num} \
    ip_param_list \{${ipParamList}\} \
    ip_port_list \{${ipPortList}\} \
}"
} else {
puts "@W \[IMPL-118\] Cannot find ::IP::FFT::gen, check your lib."
}
}


# === end of IP Block fft<ZenithFFTConfig> ===
